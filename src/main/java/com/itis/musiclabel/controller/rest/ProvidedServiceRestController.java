...
public class ProvidedServiceRestController {
    ...
    public ResponseEntity<List<ProvidedService>> getAll() {
        ...
        services.forEach(s -> {
            if (s.getLabel() != null) {
                s.setLabelName(s.getLabel().getLabelName());
            }
        });
    ...
    public ResponseEntity<ProvidedService> getById(@PathVariable Long id) {
        ...
        if (service.getLabel() != null) {
            service.setLabelName(service.getLabel().getLabelName());
        }
    ...
    public ResponseEntity<List<ProvidedService>> getByLabel(@PathVariable Long labelId) {
        List<ProvidedService> services = providedServiceService.getServicesByLabel(labelId);
        services.forEach(s -> {
            if (s.getLabel() != null) {
                s.setLabelName(s.getLabel().getLabelName());
            }
